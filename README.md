...
Install the Ruby dependencies with `bundle install`, then install the browser test tools with `npm ci` and `npx playwright install chromium firefox webkit`. Run `npm test` before pushing. The Theme checks workflow runs the same suite on pushes to `main` and on pull requests.
