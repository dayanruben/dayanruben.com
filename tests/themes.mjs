  ...
  for (const [browserName, browserType] of Object.entries({ chromium, firefox, webkit })) {
    ...
    try {
      ...
      for (const layout of ['stacked', 'sidebar']) {
        ...
        const errors = [];
        const expectedBrowserWarnings = [/The Content Security Policy directive 'frame-ancestors' is ignored when delivered via .*meta element\./];
        page.on('pageerror', error => errors.push(error.message));
        page.on('console', message => { if (message.type() === 'error' && !expectedBrowserWarnings.some(pattern => pattern.test(message.text()))) errors.push(message.text()); });
